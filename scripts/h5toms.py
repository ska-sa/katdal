...
parser.add_option("--flag-u", type=float, default=-1.0, help="Flag points with a u coordinate that is less the given value,  A negitive value implies no flagging (default=%default)  .")
...
for scan_ind, scan_state, target in h5.scans():
    ...
    if options.flag_u >= 0:
        u = h5.u[:,np.newaxis,:]*(h5.channel_freqs[np.newaxis,:,np.newaxis]/katpoint.lightspeed)
